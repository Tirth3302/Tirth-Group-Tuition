...
const email = document.getElementById('email').value.trim();
const phone = document.getElementById('phone').value.trim();
const studentClass = document.getElementById('studentClass') ? document.getElementById('studentClass').value : '';
const message = document.getElementById('message').value.trim();
            ...
            if (errors.length > 0) {
                ...
                const scriptURL = 'https://script.google.com/macros/s/AKfycbxXj_Z-SV4UB0pBFywKPLvb4ZyAOJWSumihc_HI24oiRBKUhk2A1ulsfhbaZSt5TBQ/exec';
...
/* ============================================= */
/* IN-PAGE CALLBACK FORM HANDLER                */
/* ============================================= */
document.addEventListener('DOMContentLoaded', function() {
    const cbForm = document.getElementById('openCallbackForm');
    const cbFormMessage = document.getElementById('cbFormMessage');
    const cbSubmitBtn = cbForm ? cbForm.querySelector('.callback-submit-btn') : null;

    // PASTE YOUR EXACT SAME GOOGLE WEB APP URL HERE
    const scriptURL = 'https://script.google.com/macros/s/AKfycbxXj_Z-SV4UB0pBFywKPLvb4ZyAOJWSumihc_HI24oiRBKUhk2A1ulsfhbaZSt5TBQ/exec'; 
    ...
    if (cbForm) {
        cbForm.addEventListener('submit', e => {
            ...
            // Show sending state
            cbSubmitBtn.textContent = 'Sending...';
            ...
            // ==========================================
            // FORCE FIX: Manually grab the Class dropdown
            // ==========================================
            const cbClassDropdown = document.getElementById('cbStudentClass');
            if (cbClassDropdown) {
                formData.set('studentClass', cbClassDropdown.value); 
            }
            // ==========================================
            ...
            fetch(scriptURL, { method: 'POST', body: formData})
                ...
                .then(data => {
                    if(data.result === 'success') {
                        ...
                        cbFormMessage.textContent = "Thank you! We will get back to you shortly.";
                        cbFormMessage.style.color = "green";
                        cbFormMessage.style.display = "block";
                        
                        cbForm.reset(); // Clear form fields
                        
                        // Hide success message after 5 seconds
                        setTimeout(() => {
                            cbFormMessage.style.display = "none";
                        }, 5000);
                    } else {
                ...
                .catch(error => {
                    ...
                    cbFormMessage.textContent = "Oops! Something went wrong. Please try again.";
                    cbFormMessage.style.color = "red";
                    cbFormMessage.style.display = "block";
                })
                .finally(() => {
                    // Reset button
                    cbSubmitBtn.textContent = 'Send Message';
